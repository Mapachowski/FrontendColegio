...
const Home = ({ usuario, onLogout }) => {
  ...
  const cargarNotificaciones = async () => {
    console.log('📡 Cargando notificaciones del docente autenticado');
    setLoading(true);
    try {
      const response = await apiClient.get('/notificaciones-docentes/mis-notificaciones?leidas=false');
      console.log('📥 Respuesta del servidor:', response.data);
      if (response.data.success) {
        console.log('✅ Notificaciones recibidas:', response.data.data?.length || 0);
